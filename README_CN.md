...
[Soft2D](https://www.soft2d.tech/) 是一个实时的 2D 多材料连续体物理引擎，可以模拟流体、弹性体、雪和沙子等材料。Soft2D-for-Unity 插件为 Soft2D 提供了高层封装，并为用户提供了一系列易用的的接口，可以让用户在 Unity 中方便地实现各种物理模拟效果。本插件的详细使用方法请参考[用户教程](./Docs/Simplified_Chinese/Tutorial.md)。
